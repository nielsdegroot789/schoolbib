...
class UserController
{
    ...
    public function resetPassword(Request $request, Response $response, array $args)
    {
        try{
            ...
            $body = $this->container->get('twig')->render('twig.twig', ['token' => $token, 'name' => $name]);
    ...
    public function deleteFavoriteBooks(Request $request, Response $response, array $args) {
        $this->response = $response;
        $db = new DB();
        $contents = json_decode(file_get_contents('php://input'), true);
        $resId = $contents['data'];
